...
def process_img(file: str) -> dict:
    ...
    while size_test:
        try:
            ...
            n -= 1
    top_n_colors_tuples = [tuple(color) for color in unique_colors[top_n_colors_pos].tolist()]
    top_n_colors_dict = {k: v for (k, v) in zip(top_n_colors_tuples, counts[top_n_colors_pos].tolist())}
